...
def booking2(name):
    ...
    print("1.Coimbatore to Chennai : Rs250)\n2.Trippur to Chennai : Rs200\n3.Erode to Chennai : Rs150\n4.Salem to Chennai : Rs100\n5.exit")
    c=int(input("Enter your Destination:"))
    s=int(input("No. of seats required:"))
    if c==1: 
    ...
    for key, values in booking.items():
        if key == route[str(e)]:
            ...
            if values[i]['status']=="booked":
                print(booking[key][i]['name']+" has be booked when cancelled ticket")
    #print(booking)
def history():
